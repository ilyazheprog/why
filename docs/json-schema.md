...
Top-level fields are `schema_version`, `command`, `result`, `process`, and `diagnosis`. A diagnosis contains a confidence (`certain`, `likely`, `possible`, or `unknown`), an optional cause graph, and referenced evidence. Implemented stable IDs include `exec.command_not_found`, `exec.permission_denied`, `exec.invalid_executable`, `exec.interpreter_missing`, `network.bind.address_in_use`, `process.sigsegv`, `process.sigabrt`, and `process.sigkill`.
