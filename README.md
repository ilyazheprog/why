...
- `SIGKILL` alone is never reported as OOM. A cgroup OOM is reported as likely only when the target's cgroup v2 `oom_kill` counter increases during the traced execution.
...
This is an early release. Human wording may evolve; `schema_version`, diagnostic IDs, confidence semantics, and exit-code meanings are the machine-facing interfaces.
